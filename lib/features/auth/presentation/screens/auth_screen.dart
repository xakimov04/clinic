...
// Форматтер для российских номеров телефона с фиксированным префиксом +7
class RussianPhoneInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    if (digitsOnly.length <= 2) {
      ...
      formattedValue = '+7 (${digitsOnly.substring(2, 5)}) ${digitsOnly.substring(5)}';
    } else if (digitsOnly.length <= 10) {
      formattedValue = '+7 (${digitsOnly.substring(2, 5)}) ${digitsOnly.substring(5, 8)}-${digitsOnly.substring(8)}';
    } else {
      formattedValue = '+7 (${digitsOnly.substring(2, 5)}) ${digitsOnly.substring(5, 8)}-${digitsOnly.substring(8, 10)}-${digitsOnly.substring(10)}';
    }
        ...
        children: [
          ...
          Positioned(
            ...
            child: Container(
              ...
              height: 180,
              decoration: BoxDecoration(
                color: ColorConstants.accentColor.withOpacity(0.08),
                shape: BoxShape.circle,
              ),
            ),
          ),
    ...
    return SizedBox(
      ...
      child: OAuthWidget(
        ...
        onAuth: (provider, data) {
          developer.log('AuthData: $data', name: 'AuthScreen');
          if (data != null) {
            _onVkAuth(data);
          }
        },
